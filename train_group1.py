...
def main():
    ...
    res_dir = "group1_results"
    os.makedirs(res_dir, exist_ok=True)

    # 开启 augment=True 找回 0.917
    train_ds = PH2Dataset("PH2_dataset_final/train/images", "PH2_dataset_final/train/masks", augment=True)
    ...
    # 建议 50 轮以充分收敛
    epochs = 50
    print(f">>> 开始 Group 1 完整训练 (Epochs: {epochs})...")
    for epoch in range(epochs):
        ...
        if avg_v_dice > best_dice:
            ...
            torch.save(model.state_dict(), "best_model_group1.pth")
            print(f"Epoch {epoch+1}: Best Val Dice {best_dice:.4f}")
    ...
    plt.xlabel('Epochs'); plt.ylabel('Dice'); plt.legend()

    plt.tight_layout()
    plt.savefig(f"{res_dir}/loss_and_metrics_curve.png")

    # 可视化测试
    model.load_state_dict(torch.load("best_model_group1.pth"))
    test_dices, test_ious = [] , []

    # 重要：最终评估时，可以对比 model.eval() 模式下的结果，通常会比 MC Dropout 模式更高更稳
    model.eval()
    print("\n>>> 开始最终测试集评估...")
    for idx, (img, mask) in enumerate(test_loader):
        img_gpu = img.to(device)
        
        # 为了生成热力图，这里调用 MC Dropout
        mu, sigma = mc_dropout_inference(model, img_gpu, T=10)
        
        # 为了保证指标最高，可以对比直接 eval 输出的结果
        with torch.no_grad():
            model.eval()
            eval_pred = model(img_gpu)
        
        dice, iou = get_metrics(mask.to(device), eval_pred)
        test_dices.append(dice); test_ious.append(iou)
        ...
        cv2.imwrite(f"{res_dir}/{idx}_img.png", img_bgr)
        cv2.imwrite(f"{res_dir}/{idx}_overlay.png", img_overlay)
        cv2.imwrite(f"{res_dir}/{idx}_uncertainty.png", heatmap)

    print(f"\nGroup 1 最终成绩 (eval 模式) -> Dice: {np.mean(test_dices):.4f}, IoU: {np.mean(test_ious):.4f}")
